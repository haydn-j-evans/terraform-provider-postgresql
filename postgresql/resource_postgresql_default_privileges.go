...
func resourcePostgreSQLDefaultPrivilegesRead(db *DBConnection, d *schema.ResourceData) error {
	...
	return readRoleDefaultPrivileges(txn, d, db.version)
}
...
func readRoleDefaultPrivileges(txn *sql.Tx, d *schema.ResourceData, version semver.Version) error {
...
`
	...
	privilegesEqual := resourcePrivilegesEqual(privilegesSet, d, version)
