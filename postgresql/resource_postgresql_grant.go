...
func resourcePostgreSQLGrantRead(db *DBConnection, d *schema.ResourceData) error {
	...
	return readRolePrivileges(txn, d, db.version)
}
...
func readDatabaseRolePrivileges(txn *sql.Tx, d *schema.ResourceData, roleOID uint32, version semver.Version) error {
...
`
	...
	if !resourcePrivilegesEqual(granted, d, version) {
		return d.Set("privileges", granted)
	}
	return nil
}

func readSchemaRolePrivileges(txn *sql.Tx, d *schema.ResourceData, roleOID uint32, version semver.Version) error {
...
func readForeignDataWrapperRolePrivileges(txn *sql.Tx, d *schema.ResourceData, roleOID uint32, version semver.Version) error {
...
func readForeignServerRolePrivileges(txn *sql.Tx, d *schema.ResourceData, roleOID uint32, version semver.Version) error {
	objects := d.Get("objects").(*schema.Set).List()
...
func readRolePrivileges(txn *sql.Tx, d *schema.ResourceData, version semver.Version) error {
	role := d.Get("role").(string)
	objectType := d.Get("object_type").(string)
	objects := d.Get("objects").(*schema.Set)
	...
	switch objectType {
	case "database":
		return readDatabaseRolePrivileges(txn, d, roleOID, version)

	case "schema":
		return readSchemaRolePrivileges(txn, d, roleOID, version)

	case "foreign_data_wrapper":
		return readForeignDataWrapperRolePrivileges(txn, d, roleOID, version)

	case "foreign_server":
		return readForeignServerRolePrivileges(txn, d, roleOID, version)
...
`
	...
	for rows.Next() {
		...
		privilegesSet := pgArrayToSet(privileges)
		if !resourcePrivilegesEqual(privilegesSet, d, version) {
			// If any object doesn't have the same privileges as saved in the state,
			// we return its privileges to force an update.
			log.Printf(
				"[DEBUG] %s %s has not the expected privileges %v for role %s",
